...
public class MsgBase
{
    ...
    //解码
    public static MsgBase FromBytes(byte[] bytes, int offset, int count, out int msgTotalLen)
    {
        msgTotalLen = 0;
        ...
        offset += 2;
        count -= 2;
        if (msgLen > count+2) return null;

        MsgType type = (MsgType)ReadInt16(bytes, offset, count);
        offset += 2;
        count -= 2;
        string s = System.Text.Encoding.UTF8.GetString(bytes, offset, msgLen);
        msgTotalLen = msgLen + 4;
        MsgBase msgBase = (MsgBase)JsonUtility.FromJson(s, Type.GetType(type.ToString()));
    ...
    //解码协议名（2字节长度+字符串）
    public static Int16 ReadInt16(byte[] bytes, int offset, int count)
    {
        //必须大于2字节
        if (count < 2)
        {
